(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.rewrite = function(tokens) {
      ...
      this.disambiguateContractSig();
      this.addImplicitIndentation();
      this.tagPostfixConditionals();
      this.addImplicitBraces();
      this.addImplicitParentheses();
      this.cleanContractExpr();
      return this.tokens;
    };
    ...
    Rewriter.prototype.addImplicitIndentation = function() {
      var action, condition, inContractExpression, indent, outdent, starter;
      inContractExpression = false;
      starter = indent = outdent = null;
      condition = function(token, i) {
        var _ref;
        return token[1] !== ';' && ((_ref = token[0], __indexOf.call(SINGLE_CLOSERS, _ref) >= 0) || (inContractExpression && token[0] === ',')) && !(token[0] === 'ELSE' && (starter !== 'IF' && starter !== 'THEN'));
      };
      action = function(token, i) {
      ...
      return this.scanTokens(function(token, i, tokens) {
        var tag, _ref, _ref2;
        tag = token[0];
        if (tag === 'CONTRACT_SIG') inContractExpression = true;
        if (tag === 'TERMINATOR') inContractExpression = false;
    ...
    Rewriter.prototype.disambiguateContractSig = function() {
      return this.scanTokens(function(token, i, tokens) {
        var _ref;
        if (!(token[0] === '::' && (token.spaced || token.newLine) && ((_ref = tokens[i - 1]) != null ? _ref.spaced : void 0))) {
          return 1;
        }
    ...
    Rewriter.prototype.cleanContractExpr = function() {
      var inContract;
      inContract = false;
      return this.scanTokens(function(token, i, tokens) {
        if (token[0] === 'CONTRACT_SIG') inContract = true;
        if (token[0] === 'TERMINATOR') inContract = false;
        if (token[1] === '@' && inContract && tokens[i + 1][0] === "{") {
          token[0] = 'THIS_CONTRACT';
        }
        return 1;
      });
    };
