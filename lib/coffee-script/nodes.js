(function() {
  var Access, Arr, ArrayContract, Assign, Base, Block, CONTRACT_PREFIX, Call, Class, Closure, Code, Comment, ContractOp, ContractValue, EscapeContract, Existence, Extends, For, FunctionContract, IDENTIFIER, IDENTIFIER_STR, IS_STRING, If, In, Index, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, Literal, METHOD_DEF, NEGATE, NO, Obj, ObjectContract, Op, OptionalContract, Param, Parens, RESERVED, Range, RestContract, Return, SIMPLENUM, STRICT_PROSCRIBED, Scope, SelfContract, Slice, Splat, Switch, TAB, THIS, ThisContract, Throw, Try, UTILITIES, Value, While, WrapContract, YES, compact, del, ends, extend, flatten, last, makeObjectProp, merge, multident, starts, unfoldSoak, utility, _ref, _ref1,
  ...
  exports.Block = Block = (function(_super) {
    ...
    Block.prototype.compileRoot = function(o) {
      var code, exp, i, loadContracts, loadContractsMin, moduleShim, prelude, preludeExps, rest;
      o.indent = o.bare ? '' : TAB;
      ...
      if (o.contracts) {
        return "" + prelude + " (" + moduleShim + "(function(__contracts) { " + loadContractsMin + " (function(define, require, exports) {\n  " + code + "\n}).call(this, __define, __require, __exports); }));";
      } else {
  ...
  exports.EscapeContract = EscapeContract = (function(_super) {

    __extends(EscapeContract, _super);

    function EscapeContract(value) {
      this.value = value;
    }

    EscapeContract.prototype.children = ['value'];

    EscapeContract.prototype.compileNode = function(o) {
      return this.value.compile(o);
    };

    return EscapeContract;

  })(Base);

  exports.OptionalContract = OptionalContract = (function(_super) {

    __extends(OptionalContract, _super);

    function OptionalContract(value) {
      this.value = value;
    }

    OptionalContract.prototype.children = ['value'];

    OptionalContract.prototype.compileNode = function(o) {
      return "" + (o.contracts ? CONTRACT_PREFIX : '') + "opt(" + (this.value.compile(o)) + ")";
    };
  ...
  exports.ContractOp = ContractOp = (function(_super) {

    __extends(ContractOp, _super);

    function ContractOp(op, first, second) {
      this.op = op;
      this.first = first;
      this.second = second;
    }

    ContractOp.prototype.children = ['first', 'second'];

    ContractOp.prototype.compileNode = function(o) {
      if (this.op === '||') {
        return "" + (o.contracts ? CONTRACT_PREFIX : '') + "or(" + (this.first.compile(o)) + ", " + (this.second.compile(o)) + ")";
      } else if (this.op === '&&') {
        return "" + (o.contracts ? CONTRACT_PREFIX : '') + "and(" + (this.first.compile(o)) + ", " + (this.second.compile(o)) + ")";
      } else {
        throw new SyntaxError("Unknown contract operator '" + this.op + "'");
      }
    };
  ...
  exports.SelfContract = SelfContract = (function(_super) {

    __extends(SelfContract, _super);

    function SelfContract() {}

    SelfContract.prototype.compileNode = function(o) {
      return "" + (o.contracts ? CONTRACT_PREFIX : '') + "self";
    };
  ...
  exports.WrapContract = WrapContract = (function(_super) {

    __extends(WrapContract, _super);

    function WrapContract(contract) {
      this.contract = contract;
    }

    WrapContract.prototype.children = ['contract'];

    WrapContract.prototype.compileNode = function(o) {
      if (o.contracts) {
        return "(" + (this.contract.compile(o)) + ").toContract()";
      } else {
        return "(" + (this.contract.compile(o)) + ")";
      }
    };
  ...
  makeObjectProp = function(name, value) {
    return new Assign(new Value(new Literal(name)), new Value(new Literal(value)), 'object');
  };

  exports.ThisContract = ThisContract = (function(_super) {

    __extends(ThisContract, _super);

    function ThisContract(val) {
      this.val = val;
    }

    ThisContract.prototype.children = ['val'];

    ThisContract.prototype.compileNode = function(o) {
      return this.val.compile(o);
    };
  ...
  exports.FunctionContract = FunctionContract = (function(_super) {

    __extends(FunctionContract, _super);

    function FunctionContract(dom, rng, tags, opts) {
      var dom, tc, thisContract;
      this.dom = dom;
      this.rng = rng;
      this.tags = tags;
      opts = opts || [];
      thisContract = ((function() {
        var _i, _len, _ref2, _results;
        if (dom instanceof ThisContract) {
          _ref2 = this.dom;
          _results = [];
          for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
            dom = _ref2[_i];
      ...
      if ((thisContract != null ? thisContract.length : void 0) > 1) {
        throw new SyntaxError("too many 'this' contracts");
      }
      if ((thisContract != null ? thisContract.length : void 0) === 1) {
        tc = new Assign(new Value(new Literal("this")), thisContract[0].val, 'object');
        opts = opts.concat(tc);
      }
      this.options = new Obj(opts);
      this.dom = new Arr(((function() {
        var _i, _len, _ref2, _results;
        if (!(dom instanceof ThisContract)) {
          _ref2 = this.dom;
          _results = [];
    ...
    FunctionContract.prototype.children = ['dom', 'rng', 'options'];

    FunctionContract.prototype.compileNode = function(o) {
      var params, paramsName, range, _ref2, _ref3;
      params = this.dom.compile(o);
      range = this.rng.compile(o);
      if (this.rng instanceof WrapContract && this.rng.contract instanceof Code) {
        if (((_ref2 = this.rng) != null ? (_ref3 = _ref2.contract) != null ? _ref3.params[1] : void 0 : void 0) != null) {
          paramsName = this.rng.contract.params[1].compile(o);
          delete this.rng.contract.params[1];
          this.rng.contract.params.length = 1;
        } else {
          paramsName = "";
        }
        range = this.rng.compile(o);
      ...
      if (this.tags === 'callOnly') {
        this.options.properties.push(makeObjectProp("callOnly", "true"));
      } else if (this.tags === 'newOnly') {
        this.options.properties.push(makeObjectProp("newOnly", "true"));
      } else if (this.tags === 'newSafe') {
        this.options.properties.push(makeObjectProp("newSafe", "true"));
      }
      return "" + (o.contracts ? CONTRACT_PREFIX : '') + "fun(" + params + ", " + range + ", " + (this.options.compile(o)) + ")";
    };
  ...
  exports.RestContract = RestContract = (function(_super) {

    __extends(RestContract, _super);

    function RestContract(contract) {
      this.contract = contract;
    }

    RestContract.prototype.children = ['contract'];

    RestContract.prototype.compileNode = function(o) {
      return "" + (o.contracts ? CONTRACT_PREFIX : '') + "___(" + (this.contract.compile(o)) + ")";
    };
  ...
  exports.ObjectContract = ObjectContract = (function(_super) {

    __extends(ObjectContract, _super);

    function ObjectContract(oc, opts) {
      this.oc = oc;
      this.options = opts || new Obj([]);
    }

    ObjectContract.prototype.children = ['oc'];

    ObjectContract.prototype.compileNode = function(o) {
      return "" + (o.contracts ? CONTRACT_PREFIX : '') + "object(" + (this.oc.compile(o)) + ", " + (this.options.compile(o)) + ")";
    };
  ...
  exports.ArrayContract = ArrayContract = (function(_super) {

    __extends(ArrayContract, _super);

    function ArrayContract(arc) {
      this.arc = arc;
    }

    ArrayContract.prototype.children = ['arc'];

    ArrayContract.prototype.compileNode = function(o) {
      return "" + (o.contracts ? CONTRACT_PREFIX : '') + "arr(" + (this.arc.compile(o)) + ")";
    };
  ...
  exports.ContractValue = ContractValue = (function(_super) {

    __extends(ContractValue, _super);

    function ContractValue(contract, value, contract_var, value_var) {
      this.contract = contract;
      this.value = value;
      this.contract_var = contract_var;
      this.value_var = value_var;
    }

    ContractValue.prototype.children = ['contract', 'value'];

    ContractValue.prototype.compileNode = function(o) {
      if (!(this.contract_var.base.value === this.value_var.base.value)) {
        throw new SyntaxError("Variable name differs between value (" + this.value_var.base.value + ") and contract (" + this.contract_var.base.value + ")");
      }
      if (o.contracts) {
        return "" + (o.contracts ? CONTRACT_PREFIX : '') + "guard(" + (this.contract.compile(o, LEVEL_PAREN)) + "," + (this.value.compile(o, LEVEL_PAREN)) + ")";
      } else {
        return "" + (this.value.compile(o, LEVEL_PAREN));
      }
    };
  ...
  exports.Assign = Assign = (function(_super) {
    ...
    function Assign(variable, value, context, options) {
      ...
      this.escapeContract = this.value instanceof EscapeContract;
    ...
    Assign.prototype.compileNode = function(o) {
      ...
      if (this.escapeContract && (!o.contracts)) {
        return "";
      }
